...
describe('settings compact flow', () => {
  ...
  it('renders compact Stripe drawer status labels and keeps secret metadata masked', () => {
    ...
    expect(workspace).toContain('Save API keys')
    expect(workspace).toContain('Save webhook secret')
    ...
    expect(workspace).toContain('Saved: {stripeCredentialMaskMap.PUBLISHABLE_KEY}')
    ...
    expect(workspace).toContain("publishableKey: providerForms.STRIPE.publishableKey.trim() || undefined")
    expect(workspace).toContain("secretKey: providerForms.STRIPE.secretKey.trim() || undefined")
    expect(workspace).toContain("mode: providerForms.STRIPE.mode")
    expect(workspace).toContain("webhookSecret: providerForms.STRIPE.webhookSecret.trim() || undefined")
