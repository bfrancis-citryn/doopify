  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      frame = requestAnimationFrame(() => {
        const spotlightHost = event.target instanceof Element
          ? event.target.closest<HTMLElement>('.admin-spotlight')
          : null;
    ...
    const handlePointerLeave = () => clearActive();

    const handlePointerDown = (event: PointerEvent) => {
      const spotlightHost = event.target instanceof Element
        ? event.target.closest('.admin-spotlight')
        : null;
      if (!spotlightHost) clearActive();
    };
