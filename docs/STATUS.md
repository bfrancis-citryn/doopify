...
- shipping and delivery functional expansion with persisted shipping mode/provider-usage, package/location/manual-rate/fallback-rate APIs, compact Settings -> Shipping drawer UX, checkout mode-aware live/manual/hybrid rate resolution, and order-level selected shipping method snapshots (`shippingMethodName`, `shippingRateType`, `shippingAmount`, `shippingProvider`, `shippingProviderRateId`, `estimatedDeliveryText`)
