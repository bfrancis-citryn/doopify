...
- Shipping delivery functional expansion: persisted shipping mode/provider usage and package/location/manual-rate/fallback-rate management, compact drawer-based Settings -> Shipping UX, checkout mode-aware live/manual/hybrid rate resolution with fallback-only-on-live-failure semantics, and order-level selected shipping method snapshot persistence
